...
public class CollisionHandler implements ContactListener
{
    ...
    private void processContact(Contact contact)
    {
        ...
        if (objA instanceof Main)
        {
        	processMainContact(fixtureA, fixtureB);
        }
        else if (objB instanceof Main)
        {
        	processMainContact(fixtureB, fixtureA);
        }
    }

    private void processMainContact(Fixture MainFixture, Fixture objFixture)
    {
    	if (objFixture.getBody().getUserData() instanceof Tiles)
    	{
    		Main Main = (Main)MainFixture.getBody().getUserData();
    	    Main.acceleration.y = 0;
    	    Main.velocity.y = 0;
    	    Main.jumpState = JUMP_STATE.GROUNDED;
    	    MainFixture.getBody().setLinearVelocity(Main.velocity);
    	}
    	else if (objFixture.getBody().getUserData() instanceof Beer)
    	{

    		Beer beer = (Beer)objFixture.getBody().getUserData();

    		// Remove the block update the Main's score by 1.
    		world.score += 250;
    		AudioManager.instance.play(Assets.instance.sounds.pickupCoin);


    		world.flagForRemoval(beer);
    	}

    	else if (objFixture.getBody().getUserData() instanceof Diploma)
    	{
    		Diploma d = (Diploma)objFixture.getBody().getUserData();

    		// Remove the block update the Main's score by 1.
    		world.score += d.getScore();
    		AudioManager.instance.play(Assets.instance.sounds.pickupCoin);

    		world.flagForRemoval(d);
    	}
    }
